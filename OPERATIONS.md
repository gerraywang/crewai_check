...
```mermaid
sequenceDiagram
    participant BA as Billing Analyst
    participant CO as Cloud Optimizer
    participant FO as FinOps Operator
    participant GCS as Google Cloud Storage
    participant GC as Google Calendar

    BA->>BA: 1. データリサーチ (Research Phase)<br/>課金データの調査・アノマリー特定
    BA->>CO: 分析結果の共有
    CO->>CO: 2. 戦略立案 (Strategy Phase)<br/>削減アクション策定・優先順位付け
    CO->>FO: 最適化プランの共有
    FO->>FO: 3. 成果物作成 (Reporting Phase)<br/>レポート生成 (Markdown)
    FO->>GCS: レポートの格納準備
    FO->>GC: 次週会議日程のシミュレーション
```
...
```mermaid
graph TD
    Mon[月曜: AI分析 & レポート生成] --> Tue[火曜: 技術・財務レビュー]
    Tue --> Wed[水曜: コストレビュー会議]
    Wed --> ThuFri[木曜-金曜: 最適化の実行]
    ThuFri -->|次週へ| Mon
```
